...
# --- File paths and dataset parameters ---
...
CONFIG_FILE_PATH = "configs/showo_256.yaml"
DATASET_RESOLUTION = 256
TEXT_MAX_LENGTH = 8000

# --- Training loop hyperparameters ---
NUM_TRAIN_INFERENCES = 16 # Changed from 8 to 16 as in the original code in main function
MMU_SEGMENT_NUM = 1
T2I_REF_NUM = 1
IGNORE_ID = -100
MAX_NEW_TOKENS = 16
MAX_NEW_SEQ_LEN = 512
EMA_ALPHA = 0.9
SAMPLE_STEPS = 16 # Defined here, used to be args.num_train_inferences which defaults to 8, but 16 is used in main loop
LOSS_WEIGHT_MMU = 0.5 # Weight for MMU loss
LOSS_WEIGHT_LM = 0.1 # Weight for LM loss
LOSS_WEIGHT_L1 = 0.05 # Weight for L1 loss component
GUIDANCE_SCALE_DEFAULT = 1.75 # Default guidance scale value
...
class CustomDataset(Dataset):#继承data.Dataset
    def __init__(self,t2i_path):
        ...
        def transform(example, resolution=DATASET_RESOLUTION, normalize=True):
            image=example["image"]
...
def main(args):
    ...
    config.training.guidance_scale = GUIDANCE_SCALE_DEFAULT # Use defined default value

    steps=args.num_train_inferences
    steps=SAMPLE_STEPS # Use defined SAMPLE_STEPS
    segment_num=MMU_SEGMENT_NUM # Use defined SEGMENT_NUM
    t2i_ref_num=T2I_REF_NUM # Use defined T2I_REF_NUM
    ...
    loss3_ema=0.0
    loss_mmu_ema=0.0
    ema_alpha=EMA_ALPHA # Use defined EMA_ALPHA
    step_losses = {}
    num_pred_dict = {}
    num_hope_dict = {}
    acc_dict = {}
    ignore_len=seq_len+4
    ignore_id=IGNORE_ID # Use defined IGNORE_ID
    max_new_tokens=MAX_NEW_TOKENS # Use defined MAX_NEW_TOKENS
    max_new_seq_len=MAX_NEW_SEQ_LEN # Use defined MAX_NEW_SEQ_LEN
    use_gt=False
        ...
        with accelerator.accumulate(model):
            ...
            while iitr * max_new_tokens < max_new_seq_len and eeos_reached==False:
                ...
                if unknown_map_r.sum()>0:
                    loss3=LOSS_WEIGHT_L1*loss1+loss2 # Use defined LOSS_WEIGHT_L1
                else:
                    loss3=loss2
                ...
                loss=loss3+LOSS_WEIGHT_LM*loss_lm+LOSS_WEIGHT_MMU*loss_mmu # Use defined LOSS_WEIGHT_LM and LOSS_WEIGHT_MMU
